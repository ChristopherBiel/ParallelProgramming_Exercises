...
void working_thread(int startpixel, int endpixel, auto& image_data, Checksum& checksum, const std::vector<Sphere>& copy_of_spheres, const Camera& camera)
{
    ...
    //mutex.lock();
    //std::cout << "Thread processing pixel: " << startpixel << " to " << endpixel << " " << std::endl;
    //mutex.unlock();
    for(int x = startpixel; x < endpixel; x++) {
        for(int y = height - 1; y >= 0; y--) {
            ...
            for(int s = 0; s < samples; s++) {
                ...
                pixel_color += trace_ray(r, copy_of_spheres, depth);
            }
...
int main(int argc, char **argv) {
    ...
    for (int thread_id = 0; thread_id < THREAD_NUM; thread_id++){
        int startpixel = thread_id*IMAGE_WIDTH/THREAD_NUM;
        int endpixel = (thread_id+1)*IMAGE_WIDTH/THREAD_NUM;
        threads[thread_id] = std::thread([startpixel, endpixel, &image_data, &checksum, &spheres, &camera](){
            working_thread(startpixel, endpixel, image_data, checksum, spheres, camera);
        });
    }
